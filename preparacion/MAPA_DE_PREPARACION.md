> **DOCUMENTO DE TRABAJO — no es entregable oficial.** Borrador interno de preparación. Los entregables oficiales son los 2 documentos + 2 hojas generados desde data/ (ver CLAUDE.md).

# Mapa de Preparación ETC 88 — todo lo que hay que preparar

> Consolidado del **Documento de Asesores (38 workstreams)** + **Guía canónica LXXXV** + lo que ya construimos.
> **Leyenda:** ✏️ = puedo prepararte un borrador ya · 🤝 = decisión/gestión humana · 🔢 = números/cotización humana · ⛪ = litúrgico/externo.
> **Prioridad:** 🔴 bloqueante · 🟠 antes de F1 (14-jun) · 🟡 etapa 1 (→30-jun) · ⚪ más adelante.

---

## ⭐ Lo que puedo arrancar YA (dame luz verde)
Borradores que puedo entregarte sin depender de decisiones abiertas:
1. **Guía de Guías 88** y **Horario 88** (adaptados de la base LXXXV — solo cambian fechas/directores/sede).
2. **Reglas de la Casa 88** (1 página, presentación del viernes).
3. **Formulario de inscripción del participante** (campos + salud). *Solo mayores de edad; el ETC no admite menores.*
4. **Carta de invitación a participantes** (borrador; el núcleo kerigmático lo afina Frank/AE).
5. **Lista de materiales del retiro con cantidades** (derivada del horario y las dinámicas).
6. **Botiquín base + plantilla de plan de emergencia** (ya tengo el botiquín del tablero).
7. **Plantilla de presupuesto por área** (para que coords la llenen en F1).
8. **Plantilla de tracking de pagos** (equipo + participantes + padrinos).
9. **Pitch + carta + recibo de padrinos.**
10. **Tabs nuevos del tablero:** Cantera, Presupuesto comparativo (ETC86 vs 88), Checklist del Informe 85.

---

## A. Documentos canónicos adaptados al 88
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Guía de Guías 88 (de base LXXXV) | ✏️ | Coord. Guías | 🟡 28-jun (F2) |
| Horario 88 (de base LXXXV) | ✏️ | Co-Dir + CG | 🟡 28-jun (F2) |
| Guía ETC 88 completa (institucional) | ✏️ | Co-Dir | 🟡 |
| Reglas de la Casa 88 | ✏️ | Co-Dir | 🟠 |
| Anexo A de los Estatutos (conseguir/derivar) | 🤝 | Co-Dir/Consejo | 🟠 14-jun |

## B. Formación (5 sesiones)
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| **Tema espiritual del retiro (lema + cita)** | 🤝 | Co-Dir + AE + Consejo | 🔴 esta semana |
| Plan formativo: 5 líneas (de 20) + ponentes | 🤝✏️ | Co-Dir | 🟠 7-jun |
| Mapa tema × formación (F1–F5) | ✏️ | Co-Dir + AE | 🟠 5-jun |
| Material F1 (sentido + temática + presupuesto) | ✏️ | Co-Dir | 🟠 13-jun |
| Rito de inicio del equipo (F1) | ✏️ | AE + Co-Dir | 🟠 14-jun |
| Tema de vestimenta/ambientación | 🤝 | Co-Dir + equipo | 🟡 30-jun |

## C. Equipo / roster
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Cierre nominal (7 por decidir + cupo varón) | 🤝 | Co-Dir | 🔴 |
| Parejas de guías color-coded (7 parejas) | ✏️🤝 | Coord. Guías | 🟠 |
| Equipos auxiliares (5): Recaudación/Donaciones · Guagua · Profondo · Intersección (diáspora) · Finanzas/Tesorería | 🤝 | Co-Dir | 🟡 30-jun |
| 6 roles vacantes (RF, RR, RS, RM, RT, FT) | 🤝 | Co-Dir | 🔴 |
| Completar formularios (Daylin, Olanlly + confirmar resto) | 🤝 | Coords | 🟠 |

## D. Finanzas
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Presupuesto por área (consolidar) | 🔢✏️ | RF + Co-Dir | 🟡 28-jun |
| Plan Profondo #1 (rifa, monto VARIABLE) | ✏️🔢 | Co-Dir + RF | 🟠 14-jun |
| **Profondo #2 (actividad + meta)** | 🤝 | Co-Dir | 🔴 30-jun |
| Plan padrinos (pitch, carta, recibo, lista) | ✏️ | Co-Dir | 🟡 30-jun |
| Tracking de pagos por persona | ✏️ | RF | 🟡 28-jun |

## E. Participantes
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Carta de invitación + protocolo de cobro | ✏️ | Co-Dir | 🟠 30-may |
| Formulario de inscripción del participante | ✏️ | Co-Dir | 🟠 |
| Lectura de perfiles (criterios + escalación) | ✏️🤝 | Guías + Co-Dir | 19-jul / 16-ago |
| Solo mayores de edad (el ETC no admite menores) — regla del director | ✔️ | Co-Dir | confirmada 3-jun |
| Carta pre-retiro a participantes | ✏️ | Co-Dir | ⚪ |

## F. Salud y emergencias
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Botiquín del retiro | ✏️ (listo) | Resp. Salud | 🟡 |
| Plan de emergencia médica (hospital Higüey + ruta) | ✏️🤝 | Resp. Salud | 🟡 30-jun |
| Recolectar aviso médico de cada persona | 🤝 | Coords | 🟠 |

## G. Casa / logística
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Visita de reconocimiento + mapa de espacios | 🤝 | Co-Dir + DC + CG | 🟡 antes 1-ago |
| Contrato/factura Samuel Montilla | 🤝 | Co-Dir | 🟡 30-jun |
| Distribución de dormitorios | 🤝 | Co-Dir | ⚪ tras visita |

## H. Transporte / Guagua
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Armar equipo de guagua (hoy vacío) | 🤝 | Co-Dir | 🟡 30-jun |
| Cotizaciones (SD→Higüey y local) | 🔢 | Resp. Transporte | 🟡 5-jun |
| Plan de movilidad (quién, qué auto, qué carga) | ✏️🤝 | Resp. Transporte | ⚪ |

## I. Litúrgico / espiritual
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Confesores sábado 9pm (≥2-3 sacerdotes) | ⛪ | AE (P. Paul) | 🟡 30-jun |
| Coordinación con párroco de Higüey | ⛪ | AE | 🟡 30-jun |
| Palancas (distribución, sobres, citas bíblicas) | ✏️🤝 | AE + Coords | ⚪ |
| Banderín (diseño + materiales) | ✏️ | Frank + 2 guías | 🟡 |
| Misa de clausura (lecturas, repertorio, peces) | ✏️⛪ | Música + AE | ⚪ |

## J. Materiales del retiro
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Lista con cantidades (biblias, peces, libretas, alambres, cartulinas, carnets, sobres, papelógrafo, hoja de datos, banderín) | ✏️ | Resp. Materiales | 🟡 5-jun |
| Camisetas del equipo (decisión viabilidad) | 🤝🔢 | Co-Dir + RM | 🟡 19-jul |

## K. Comunicación
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Grupos de WhatsApp (Equipo · por área · Co-Dir) | 🤝 | Co-Dir | 🟠 24-may |
| RRSS: opción A/B/C + calendario editorial | ✏️🤝 | Resp. RRSS | 🟠 30-may |
| Protocolo de celulares/fotos + consentimiento | ✏️ | Co-Dir | 🟡 |

## L. Post-retiro
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Bienvenida nuevos etecianos 9-sep | ✏️🤝 | Cocina + Co-Dir | ⚪ |
| Evaluación post-retiro 13-sep (encuesta + reunión) | ✏️ | Co-Dir | ⚪ |
| Reuniones dominicales (4to día) | 🤝 | Consejo | ⚪ |

## M. El tablero / datos (lo nuestro)
| Item | Prep | Dueño | Fecha |
|---|---|---|---|
| Tab "Cantera" (pool fuera del equipo) | ✏️ | — | 🟡 |
| Tab presupuesto comparativo (ETC86 vs 88) | ✏️ | — | 🟡 |
| Checklist del Informe 85 como banderas de formación | ✏️ | — | 🟡 |
| Hoja en vivo sincronizada con el tablero | ✏️ (listo) | — | ✓ |

---

## Resumen de bloqueantes 🔴 (lo que frena todo)
1. **Tema espiritual del retiro** (lema + cita) — esta semana → destraba invitación, RRSS, banderín, misa.
2. **Cierre nominal del equipo** (7 por decidir + cupo varón + 6 roles vacantes).
3. **Profondo #2** (actividad + meta) — 30-jun → cierra el presupuesto.

> Dime cuáles de los ✏️ quieres que arranque primero y te los voy entregando.
