> **DOCUMENTO DE TRABAJO — no es entregable oficial.** Borrador interno de preparación. Los entregables oficiales son los 2 documentos + 2 hojas generados desde data/ (ver CLAUDE.md).

# Documento de Seguimiento ETC 88
### Vivo · se actualiza en cada decisión/acción/cierre

> **Cómo funciona:** este es el doc maestro de seguimiento. Cada vez que se toma una decisión, se completa una acción, o se abre un pendiente, se anota acá con fecha y dueño.
> **Fuente de personas:** el [Sheet en vivo](https://docs.google.com/spreadsheets/d/1q4Kt_zQm_FQK_kmJh5lrh_tf_yejFPdEMvjnenre7AI). Si el sheet cambia, este doc se regenera.
> **Versión actual:** v7 — 2026-06-02.

---

## 1. Estado del equipo (cifras)
- **Total manifiesto:** 56 personas.
- **Operativos (formación + retiro):** 47 (incluye Paul + Sor transversales).
- **Ampliados (solo retiro):** 5 (asesoras cocina + asesoras comunidad).
- **Backups:** 3 (guías).
- **Vacante:** 1 (cocina, varón, candidato Randolph).
- **Sin formulario:** 4 (Daylin · Olanlly · Pamela · Frank — y los placeholders nuevos).

## 2. DECISIONES tomadas

| Fecha | Decisión | Tomada por |
|---|---|---|
| 2-jun | **Constelación** quitada del branding | Co-Dir |
| 2-jun | **Leober Carrión Soriano** (typo "Soriano") | Usuario |
| 2-jun | **Roselyn fuera** · vacante para varón (candidato Randolph) | Co-Dir |
| 2-jun | Backups guías: **Rodolfo · Scarlett · Kamila Todd** | Co-Dir |
| 2-jun | **Recaudación y Donaciones = 1 equipo** | Co-Dir |
| 2-jun | **Guagua = equipo de transporte** (no tarea) | Co-Dir |
| 2-jun | Equipo **Intersección (espiritual)** = nuevo auxiliar | Co-Dir |
| 2-jun | **Petra + Johanny** confirmadas (asesoras cocina) | Co-Dir |
| 2-jun | **Paul + Sor = transversales** (todo el proceso) | Co-Dir |
| 2-jun | **Ricaira / Mirna / Fabianlly / Kirina NO están** | Co-Dir |
| 2-jun | **Reunión coords = jue 4-jun virtual** (Corpus) | Coords |
| 2-jun | **Convivencia 22-ago** inicia con miniretiro/reflexión | Co-Dir |
| 2-jun | **Branding se reconstruye desde 0** desde la raíz espiritual | Co-Dir |
| 2-jun | Temática confirmada: **expedición / búsqueda de tesoro / aventura** | Co-Dir |
| 2-jun | **Frank = asesor + lleva el Banderín** | Co-Dir |
| 2-jun | **Fabelly = primer ETC** | Co-Dir |
| 2-jun | **Cuota equipo y participante = RD$3,000** | Doc Asesores v10 |
| 2-jun | **Casa tiene sonido** → música no renta | Co-Dir |

## 3. ACCIONES en curso (esta semana)

| Acción | Dueño | Fecha límite | Estado |
|---|---|---|---|
| **Cerrar equipo nominal + enviar invitaciones** | JM + JC | mié 3-jun | 🟡 en curso |
| **Definir raíz espiritual (hilo + lema + cita)** | Co-Dir + Paul + Sor | esta semana | 🔴 abierto |
| **Reunión coords virtual** | Todos los coords | jue 4-jun | 📅 programada |
| **Diseñar Programa F1** | Co-Dir + AE | antes 14-jun | 🟡 borrador listo |
| **Asignar responsables de los 4 auxiliares** | Co-Dir | antes F1 | 🔴 abierto |
| **Confirmar invitaciones aceptadas** | Cada coord | esta semana | 🟡 en curso |
| **Pre-lectura enviada a coords** | Co-Dir | mar 2-jun | ✅ listo en repo |
| **Subir docs al Drive** | Co-Dir (yo lo hago) | hoy | 🟡 |

## 4. PENDIENTES (con prioridad)

### 🔴 Bloqueantes
- [ ] Raíz espiritual definitiva (hilo principal + secundario)
- [ ] Confirmar varón para cocina (Randolph u otro)
- [ ] Confirmar Pamela (sin formulario)
- [ ] Asignar responsables de los 4 equipos auxiliares
- [ ] **Profondo #2** (actividad + meta) — deadline 30-jun
- [ ] Confirmar **La Vega: 1 o 2** asesores

### 🟠 Antes de F1 (14-jun)
- [ ] Plantilla de presupuesto por área (enviar a coords)
- [ ] Borrador Programa F1
- [ ] **Formulario de Perfil del Participante** (perfilación temprana)
- [ ] Reglas de la Casa 88
- [ ] Guía 88 + Horario 88 (adaptar de LXXXV)

### 🟡 Etapa 1 (→ 30-jun)
- [ ] **Lugar/salón de las 5 formaciones** (sede fija/rotativa) — Co-Dir — deadline 5-jun (del Doc de Asesores)
- [ ] **Lugar para reuniones de Dirección** (recurrente)
- [ ] Cartas a donantes (Recaudación)
- [ ] 3 cotizaciones de transporte (Guagua)
- [ ] Plan de rifa Profondo #1
- [ ] Visita a la casa de retiro
- [ ] Confirmar precio casa 2026 + exención
- [ ] Carpetas por área (Cocina/Guías/Música)
- [ ] **Adiestramiento en Liturgia** de guías
- [ ] Planificación del Banderín (Frank + 2 guías)
- [ ] Música: canción de despedida + dinámicas + cancionero
- [ ] Cocina: menú definitivo + correcaminos

### 🟢 Banderas operativas
- [ ] Contacto de emergencia de Jonathan (falta)
- [ ] Higüey: confirmar gatos · no fumigar Baygon · limpieza polvo previa
- [ ] Botiquín: SIN penicilina/ibuprofeno/AINEs/neo-melubrina/metoclopramida
- [ ] Botiquín: CON paracetamol + inhalador + antialérgico

### 📋 Detalles extraídos del Doc de Asesores aún no integrados (relevantes)
- [ ] **Lugar de las formaciones** (sede fija/rotativa/mixta) — deadline 5-jun
- [ ] **Visita de reconocimiento a la casa** (mapa de espacios, dormitorios) — antes 1-ago
- [ ] **Contrato/factura formal** de Samuel Montilla (casa)
- [ ] **Confesores** sábado 9pm (≥2-3 sacerdotes) — gestiona Paul
- [ ] **Coordinación con párroco de Higüey** (respaldo)
- [ ] **Plan de emergencia médica** (hospital Higüey + ruta + responsable por turno)
- [ ] **Permisos firmados** para participantes menores de edad
- [ ] **Carta pre-retiro** a participantes
- [ ] **Camisetas del equipo** — decisión de viabilidad (19-jul)
- [ ] **RRSS:** opción A/B/C + calendario editorial (arranca 30-may)
- [ ] **Política de fotos/video** + consentimiento de participantes
- [ ] **Protocolo de celulares** durante el retiro
- [ ] **Acta del retiro** (memoria operativa y espiritual)
- [ ] **Encuesta de evaluación** a participantes (a la semana)
- [ ] **Lugar de reuniones dominicales** (4º día) post-retiro
- [ ] **Anexo A de los Estatutos** (conseguir/derivar al Consejo)
- [ ] **Método de pago a proveedores** + trazabilidad de comprobantes
- [ ] **Biblias, peces, materiales** (cotizar local Higüey)
- [ ] **Rúbrica de evaluación de servidores** (existe del ETC 79)
- [ ] **Bienvenida nuevos etecianos** 9-sep (cocina: baile + bizcocho)

## 5. PREGUNTAS abiertas (para resolver)

| Pregunta | A quién | Estado |
|---|---|---|
| ¿Cuál es el hilo principal de la raíz? | Co-Dir + Paul/Sor | abierto |
| ¿Randolph confirma la vacante? | JC + JM | abierto |
| ¿La Vega envía 1 o 2 asesores? | Co-Dir + La Vega | abierto |
| ¿Cuál es la actividad del Profondo #2? | Co-Dir | abierto |
| ¿Qué hacemos con los backups de guías? (sólo si baja alguien, o se integran a algo) | Co-Dir | abierto |

## 6. HITOS de fechas (lo que viene)

| Fecha | Hito |
|---|---|
| **mié 3-jun** | Cierre del equipo + invitaciones |
| **jue 4-jun** | Reunión coords (virtual) |
| dom 7-jun | Misa eteciana (sin formación) |
| **sáb 14-jun** | **F1** |
| sáb 28-jun | F2 |
| dom 5-jul | F3 |
| dom 19-jul | F4 + Perfiles 1 |
| 31-jul–2-ago | **Profondo #1 (rifa)** |
| dom 16-ago | F5 + Perfiles 2 |
| sáb 22-ago | Convivencia (con miniretiro) |
| dom 23-ago | **Ensayo General (obligatorio)** |
| dom 30-ago | Reunión final · cobranza 100% |
| jue 3-sep | Avanzada Cocina |
| **vie 4 → dom 6-sep** | **RETIRO ETC 88** |
| mié 9-sep | Bienvenida nuevos |
| dom 13-sep | Evaluación post-retiro |

---

## Cómo se mantiene vivo este documento
1. **Personas/roster:** se edita en el [Sheet en vivo](https://docs.google.com/spreadsheets/d/1q4Kt_zQm_FQK_kmJh5lrh_tf_yejFPdEMvjnenre7AI) (sección 1).
2. **Decisiones:** se anotan con fecha y quién (sección 2).
3. **Acciones:** se actualiza estado (🔴 abierto · 🟡 en curso · ✅ listo).
4. **Pendientes:** check cuando se cierra.
5. **Reuniones:** acuerdos se vuelcan acá el mismo día.

> Vamos a tener una versión en Google Doc en el Drive, conectada visualmente al tablero, para que coords y asesores la consulten sin abrir el repo.
