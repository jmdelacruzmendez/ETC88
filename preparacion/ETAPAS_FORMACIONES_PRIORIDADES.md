> **DOCUMENTO DE TRABAJO — no es entregable oficial.** Borrador interno de preparación. Los entregables oficiales son los 2 documentos + 2 hojas generados desde data/ (ver CLAUDE.md).

# Etapas · Formaciones · Prioridades — ETC 88
### Dimensión completa del proceso · qué toca a cada etapa

> Lo que el usuario pidió: dimensionar **por etapas**, **formaciones**, **prioridades**, y **qué toca en cada etapa**.
> Esto es el "mapa de operación" que sirve para los directores y para la reunión de coordinadores.

---

## 0. Estructura general (5 etapas)

| Etapa | Ventana | Foco | Hito de cierre |
|---|---|---|---|
| **0 · Cimientos** | hasta mié **3-jun** | Cierre nominal del equipo + invitaciones | Equipo cerrado (JC + JM) |
| **1 · Socialización + raíz** | **4 → 14-jun** | Reunión coords + raíz espiritual + F1 | F1 dictada |
| **2 · Construcción** | **15-jun → 19-jul** | F2–F4 + presupuestos + perfiles 1 + Profondo #1 | Perfiles 1 + Rifa |
| **3 · Consolidación** | **20-jul → 30-ago** | F5 + Convivencia + Ensayo + cobranza | Equipo al 100% |
| **4 · Ejecución** | **3 → 13-sep** | Avanzada + Retiro + Bienvenida + Evaluación | Cuarto día arranca |

---

## 1. Qué toca en CADA etapa (por área y por equipo auxiliar)

### Etapa 0 · Cimientos (hasta 3-jun) — 🔴 prioridad máxima
| Quién | Qué |
|---|---|
| **Co-Dir (JM + JC)** | Cerrar el equipo nominal (1 vacante, sin-formulario, asesores comunidad) · enviar **invitaciones formales** · responder dudas |
| Coords | Confirmar disponibilidad · recibir invitación |
| Auxiliares | Sin tarea aún |
| Asesores transversales (Paul + Sor) | Disponibles para consulta |

### Etapa 1 · Socialización + raíz (4 → 14-jun) — 🔴
| Quién | Qué |
|---|---|
| **Co-Dir** | Reunión de coords **jue 4-jun virtual** (Corpus) · cerrar **raíz espiritual** con Paul/Sor · diseñar F1 · enviar plantilla de presupuesto por área |
| **Coords (Priscilla/Camila · Paloma/Jhonnito · José)** | Pre-leer paquete · participar en reunión · **traer presupuesto + materiales + 3 dudas a F1** |
| **Equipo completo** | F1 el **14-jun** (presupuesto, sentido, temática, "carta al que fui") |
| Recaudación y Donaciones | Mapear fuentes (padrinos, donantes, empresas) |
| Guagua | Pre-cotizaciones |
| Profondo | Pre-ideación de actividad |
| Intersección | Cadena de oración semanal arranca |
| Paul + Sor | Inputs para raíz espiritual |

### Etapa 2 · Construcción (15-jun → 19-jul) — 🟠
| Quién | Qué |
|---|---|
| Co-Dir | Consolidar **el presupuesto** con los números de cada área (deadline F2 28-jun) · la **recaudación de cuotas inicia en F3 (5-jul)**; pago lump o por cuotas a elección del miembro |
| Guías | F2 (28-jun) · F3 (5-jul) · **definir parejas color-coded** · empezar testimonios · **Lectura de Perfiles 1** (19-jul) |
| Cocina | **Menú definitivo + lista de compras** (entregado en F1 · ajustado en F2) · compras anticipadas no perecederos · **identificar Correcaminos** |
| Música | **Repertorio + setlist** · ensayos (mín. 3) · aprender canción de despedida + dinámicas |
| Recaudación-Donaciones | Cartas a donantes · cierre preliminar de padrinos · campaña en RRSS |
| Guagua | **3 cotizaciones formales** · plan de movilidad borrador |
| Profondo | **Plan rifa Profondo #1** (ejecución 31-jul → 2-ago; monto VARIABLE — lo que se recaude; primera actividad por recomendación del equipo) · definir Profondo #2 |
| Intersección | Oración por cada participante mencionado · acompañamiento del equipo |
| Paul + Sor | Asesoría en testimonios · enlace litúrgico |

### Etapa 3 · Consolidación (20-jul → 30-ago) — 🟠
| Quién | Qué |
|---|---|
| Co-Dir | **F4 19-jul** (perfiles 1) · **F5 16-ago** (perfiles 2 + cierre de cobro) · validación final del equipo |
| Guías | **Formación en Liturgia + Banderín** · ensayo de pequeños grupos · adaptar Guía 88 · cerrar parejas |
| Cocina | Lista de compras finalizada · avanzada planificada · **Correcaminos confirmado** |
| Música | **Cancionero impreso** · ensayar misa de clausura · canción de despedida memorizada |
| Recaudación-Donaciones | **Profondo #1 (rifa)** 31-jul–2-ago · cierre nominal de padrinos (30-jul) |
| Guagua | Plan de movilidad **final** |
| Profondo | Profondo #2 ejecutado |
| Intersección | Oración intensiva · acompañamiento espiritual del equipo |
| Equipo completo | **Convivencia 22-ago** (inicia con miniretiro/reflexión) · **Ensayo General 23-ago** (obligatorio) · **Reunión final 30-ago** (cobranza al 100%) |

### Etapa 4 · Ejecución (3 → 13-sep) — 🟠
| Quién | Qué |
|---|---|
| Cocina | **Avanzada 3-sep** (10am) |
| Equipo completo | **Retiro 4–6-sep** |
| Cocina | **Bienvenida nuevos etecianos 9-sep** (baile + bizcocho) |
| Co-Dir + Asesores | **Evaluación 13-sep** (encuesta + reunión) |
| Intersección | Acompañamiento del 4to día |

---

## 2. Las 5 FORMACIONES (qué toca en cada una)

| F | Fecha | Eje | Entregables del equipo |
|---|---|---|---|
| **F1** | 14-jun | "El llamado a la expedición" — Identidad del servidor | Cada coord: **presupuesto de su área + materiales + 3 dudas** |
| F2 | 28-jun | "Una sola tripulación" — Unidad y motivaciones | Presupuesto consolidado · adaptación Guía/Horario 88 *(se anuncia el modelo; la recaudación arranca en F3)* |
| F3 | 5-jul | "El tesoro que buscamos" — El participante en el centro | **Arranque de la recaudación** (cuotas equipo + participantes; lump o cuotas, a elección) · compras anticipadas · setlist música · cartas donantes enviadas |
| F4 | 19-jul | "Conociendo el terreno" — Lectura de perfiles 1 | Borrador de pequeños grupos · perfiles asignados · cobro en curso |
| F5 | 16-ago | "Listos para zarpar" — Envío y consagración | Perfiles 2 cerrados · cobro al cierre · cancionero impreso · materiales completos |

---

## 3. Mapa de PRIORIDADES — qué construir antes de qué

```
SEMANA 1 (esta) ───→  cierre del equipo (mié 3-jun)
                      reunión coords (jue 4-jun virtual)
                      raíz espiritual (Co-Dir + Paul/Sor)
                      programa F1
                  ↓
SEMANA 2 ─────────→  F1 (14-jun)
                      presupuesto por área entregado
                      cada coord ya bajó el mensaje a su equipo
                  ↓
SEMANA 3-4 ───────→  F2 + F3 (28-jun, 5-jul)
                      Guía 88 adaptada
                      perfiles arrancan a entrar
                  ↓
JULIO ───────────→  F4 + Profondo #1
                      perfiles 1 leídos
                      compras anticipadas
                  ↓
AGOSTO ──────────→  F5 + Convivencia + Ensayo
                      Profondo #2
                      cobranza al 100%
                  ↓
SEPTIEMBRE ──────→  Avanzada · RETIRO · 4to día
```

**Bloqueantes (si no se cierran, frenan lo de abajo):**
1. **Cierre del equipo** (3-jun) → bloquea invitaciones.
2. **Raíz espiritual** (esta semana) → bloquea F1, branding, banderín, misa.
3. **Reunión coords** (4-jun) → bloquea la unidad de mensaje en F1.
4. **Presupuesto consolidado por área** (F1) → bloquea cobranza (que arranca en F3).

---

## 4. Quién OWN qué en cada etapa (resumen)

| Etapa | Co-Dir | Coords | Auxiliares | Paul/Sor |
|---|---|---|---|---|
| 0 | **Cierre + invita** | confirma | — | consulta |
| 1 | **Coords + F1 + raíz** | preparar | mapeo | aporte a raíz |
| 2 | F2-F4 + presupuesto | ejecutar | ejecutar | testimonios |
| 3 | F5 + Convivencia + Ensayo | finalizar | cerrar | acompañar |
| 4 | Retiro | dirigir | apoyar | presidir liturgia |

> Cada área entrega lo suyo a su coord, el coord a Co-Dir, Co-Dir consolida y Paul/Sor (transversales) acompañan en cada etapa.
