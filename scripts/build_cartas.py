...
Salidas (raíz del repo):
  1. Carta_GrupoSID_ETC88.docx       — PRIMERA carta, personalizada (rubro cocina).
...
Contactos y donantes: preparacion/DIRECTORIO_DONANTES_ETC88.md (evidencia primaria de
Drive ETC 78 + xlsx Misiones 2025 + investigación web). Cada fila lleva procedencia.
El logo se incrusta si existe data/logo_etc.png; si no, deja un marcador.
"""
import json, csv, os
...
def membrete(doc):
    ...
    _p(doc, 'San Pedro de Macorís, Rep. Dom. — [fecha]', italic=True, align=DER)
    _p(doc)

def firma(doc):
    _p(doc)
    _p(doc, '____________________            ____________________')
    names = '            '.join(CODIR[:2]) if len(CODIR) >= 2 else (CODIR[0] if CODIR else '[Co-Dirección]')
    _p(doc, names, bold=True)
    _p(doc, f'Co-Dirección · Encuentro Total con Cristo (ETC) {NUM}')
    _p(doc, '[Tel. / WhatsApp de la Co-Dirección]', italic=True)
...
def carta_grupo_sid():
    doc = new_doc()
    membrete(doc)
    destinatario(doc, ['Señores', 'GRUPO SID / MercaSID',
                       "At'n.: Departamento de Responsabilidad Social («Acciones que dejan huellas»)",
                       'Av. Máximo Gómez #182, Santo Domingo, R.D.'])
    _p(doc, SALUDO)
    _p(doc, ACTIVIDAD)
    _p(doc, 'Conocedores del compromiso social de Grupo SID con las familias dominicanas a '
            'través de la donación de productos de primera necesidad, apelamos a su generosidad '
            'solicitando nos faciliten productos de su línea —que ya forman parte de nuestro '
            'presupuesto de cocina— para alimentar a los jóvenes durante el retiro, tales como:')
    table(doc, ['Cantidad', 'Unidad', 'Artículo'], [
        ['[cant.]', 'Galones', 'Aceite comestible (Crisol / Mazola)'],
        ['[cant.]', 'Libras', 'Harina'],
        ['[cant.]', 'Libras', 'Pollo (Induveca)'],
        ['[cant.]', 'Barras', 'Salami / embutidos (Induveca)'],
    ])
    _p(doc, 'Cualquier aporte, en producto o en efectivo, será de gran ayuda. Por ser una '
            'asociación de fieles de la Diócesis de San Pedro de Macorís, podemos emitir la '
            'constancia de su donación que su empresa requiera, y con gusto lo incluimos en '
            'nuestra cadena de oración.')
    _p(doc, DESPEDIDA)
    firma(doc)
    out = f'{REPO}/Carta_GrupoSID_ETC88.docx'
...
EMPRESAS = [
    ...
    ['Alimentos', 'Induveca (pollo/embutidos)', 'servicioalcliente@induveca.com.do', 'Nacional'],
...
# --------------------------------------------------------- donantes previos ---
# Evidencia primaria: xlsx "Presupuesto Obra Misional 2025" + Drive ETC 78.
DON_EMPRESAS_2025 = [
...
# ---------------------------------------------------------------- tracker ----
TARGETS = [
    ['1', 'Grupo SID / MercaSID', 'Alimentos (aceite/pollo)', 'Empresa-rubro', 'compras@mercasid.com.do · 809-565-2151', 'web·verificar (dona producto)', 'aceite, harina, pollo, embutidos'],
    ...
    ['3', 'Induveca (Grupo SID)', 'Alimentos (pollo)', 'Empresa-rubro', 'servicioalcliente@induveca.com.do', 'web·verificar', 'pollo, salami, embutidos'],
