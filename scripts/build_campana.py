...
Ejercicio de concientización (no presentación económica). Orden narrativo:
  1. Hook        — patch Misión 88 + cita (Sal 19,1).
  2. KPIs        — costo, lo que ponemos, falta, días.
  3. Estructura  — pastel: en qué se va el costo (% por rubro).
  4. Por persona — costo participante y equipo, desglosado (pasteles).
  5. Cuotas      — lo que ya ponemos y qué cubre cada una.
  6. Simulador   — cohete: marca lo que consigamos y baja la falta. (engancha)
  7. Áreas       — corresponsabilidad.
  8. Cuenta regresiva.
  9. Presupuesto completo — desplegable con el detalle/menú (del Maestro).

Cifras desde data/estado.json; detalle del presupuesto desde el Maestro xlsx.
El control de pagos por nombre NO va aquí (Excel interno de Tesorería).
...
# ------------------------------------- detalle del presupuesto (Maestro xlsx) --
presupuesto = []
sub_operativo = 0
try:
    import openpyxl
    MAESTRO = os.path.join(REPO, 'data', 'presupuesto', 'ETC88_Presupuesto_Maestro.xlsx')
    ws = openpyxl.load_workbook(MAESTRO, data_only=True)['01 · LADO A Costos']
    groups, order = {}, []
    for row in ws.iter_rows(values_only=True):
        a = row[0]
        if not isinstance(a, str) or not a[:1].isdigit() or '·' not in a:
            continue
        item = row[2]
        if item is None:
            continue
        ...
        groups[a].append({"item": str(item), "detalle": detalle, "monto": round(monto, 2)})
    for rb in order:
        st = round(sum(i['monto'] for i in groups[rb]), 2)
        presupuesto.append({"rubro": rb, "items": groups[rb], "subtotal": st})
        sub_operativo += st
    sub_operativo = round(sub_operativo, 2)
except Exception as e:   # noqa
    print("  aviso: no se pudo leer el Maestro (", e, ") — desplegable con rubros agrupados")
    for r in sim['items']:
...
if round(sum(d['monto'] for d in desg_part), 2) != part_oper:
    errs.append("desglose participante no suma")
if round(sum(d['monto'] for d in desg_eq), 2) != eq_oper:
    errs.append("desglose equipo no suma")
if sum(r['monto'] for r in sim['items']) != meta:
    errs.append("rubros simulador ≠ meta")
...
datos = {
    ...
    "cita": cita['valor'], "citaRef": cita['cita'],
    ...
    "cuotaEquipo": {"pago": eq_pago, "pagos": eq_pagos, "total": eq_total, "n": n_equipo, "totalRecaudo": equipo_cuotas,
...
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0}
  ...
  .cita{margin:16px auto 0; max-width:640px; color:var(--gold); font-style:italic; font-size:clamp(.98rem,2.2vw,1.3rem); line-height:1.35}
  ...
  .pie{width:clamp(130px,28vw,180px); aspect-ratio:1; border-radius:50%; flex:none; box-shadow:0 0 0 6px rgba(255,255,255,.03); position:relative}
  ...
  .cd-num{font-size:clamp(1.4rem,4.5vw,2.2rem); font-weight:800; font-variant-numeric:tabular-nums; line-height:1; background:linear-gradient(180deg,#fff,#bcd2f5); -webkit-background-clip:text; background-clip:text; color:transparent}
  ...
  .pres-it{display:grid; grid-template-columns:1fr auto auto; gap:10px; align-items:center; padding:6px 0; border-top:1px solid rgba(255,255,255,.05); font-size:.88rem}
  .pi-n{color:var(--ink)} .pi-d{color:var(--muted2); font-size:.82rem; white-space:nowrap} .pi-m{font-variant-numeric:tabular-nums; color:var(--muted); font-weight:600; white-space:nowrap}
...
<div class="wrap">
  ...
  <section class="panel reveal" id="estructura-panel">
    <h2>En qué se va el costo de la misión</h2>
    <p class="h2note">La estructura completa, por rubro (incluye el 10% de imprevistos)</p>
    <div class="pie-row"><div class="pie" id="pie-estructura"></div><div class="legend" id="leg-estructura"></div></div>
  </section>
  ...
  <section class="panel reveal" id="costo-panel">
    ...
    <div class="grid2">
      <div class="costo-card">
        <div class="ct">Por participante</div><div class="cbig" id="cp-part">RD$ 0</div>
        <div class="csub">su cuota es <b id="cp-part-cuota"></b> · el resto no recae en él</div>
        <div class="pie-row"><div class="pie donut" id="pie-part"></div><div class="legend" id="leg-part"></div></div>
      </div>
      <div class="costo-card">
        <div class="ct">Por miembro de equipo</div><div class="cbig" id="cp-eq" style="color:var(--sky)">RD$ 0</div>
        <div class="csub">su cuota es <b id="cp-eq-cuota"></b></div>
        <div class="pie-row"><div class="pie donut" id="pie-eq"></div><div class="legend" id="leg-eq"></div></div>
      </div>
    </div>
  </section>
  ...
  <section class="panel reveal" id="cuota-panel">
    ...
    <div class="grid2">
      <div class="qcard"><div class="qt">Cuota del participante</div><div class="qbig" id="q-part">RD$ 0</div>
        <div class="qline" id="q-part-line"></div><div class="chips" id="q-part-chips"></div><div class="qtot" id="q-part-tot"></div></div>
      <div class="qcard"><div class="qt">Cuota del equipo</div><div class="qbig" id="q-eq" style="color:var(--green)">RD$ 0</div>
        <div class="qline" id="q-eq-line"></div><div class="chips" id="q-eq-chips"></div><div class="qtot" id="q-eq-tot"></div></div>
    </div>
  </section>
    ...
    <div class="sim-flex">
      <div class="rocket-wrap" aria-hidden="true">
        <svg viewBox="0 0 160 430">
          <defs>
            <linearGradient id="fuelG" x1="0" y1="1" x2="0" y2="0"><stop offset="0%" stop-color="#0c8fce"/><stop offset="55%" stop-color="#34D399"/><stop offset="100%" stop-color="#A7F3D0"/></linearGradient>
      ...
      <div class="sim-read">
        ...
        <div class="sim-mini"><div>por cada uno (100)<b id="sim-porcabeza">RD$ 0</b></div><div>costo de la misión<b id="sim-costo">RD$ 0</b></div></div>
  ...
  <section class="panel reveal" id="cd-panel">
    ...
    <div class="cd-two">
      <div class="cd-block urgent"><div class="cd-head">Cierre de pagos · <b id="cd-cpago-when"></b></div>
        <div class="cd-grid"><div class="cd-cell"><div class="cd-num" id="cpago-d">0</div><div class="cd-lbl">Días</div></div><div class="cd-cell"><div class="cd-num" id="cpago-h">00</div><div class="cd-lbl">Hrs</div></div><div class="cd-cell"><div class="cd-num" id="cpago-m">00</div><div class="cd-lbl">Min</div></div><div class="cd-cell"><div class="cd-num" id="cpago-s">00</div><div class="cd-lbl">Seg</div></div></div></div>
      <div class="cd-block"><div class="cd-head">Lanzamiento (retiro) · <b id="cd-ret-when"></b></div>
        <div class="cd-grid"><div class="cd-cell"><div class="cd-num" id="ret-d">0</div><div class="cd-lbl">Días</div></div><div class="cd-cell"><div class="cd-num" id="ret-h">00</div><div class="cd-lbl">Hrs</div></div><div class="cd-cell"><div class="cd-num" id="ret-m">00</div><div class="cd-lbl">Min</div></div><div class="cd-cell"><div class="cd-num" id="ret-s">00</div><div class="cd-lbl">Seg</div></div></div></div>
    </div>
  </section>

  <!-- presupuesto completo (desplegable) -->
  <section class="panel reveal" id="pres-panel">
    ...
    <p class="pres-foot">Subtotal operativo <b id="pres-sub">RD$ 0</b> + 10% de imprevistos = <b id="pres-total">RD$ 0</b> (la meta de la misión).</p>
  </section>

  <footer>
    Hecho por la tripulación · desde <b>data/estado.json</b> y el Presupuesto Maestro · actualizado <span id="ft-fecha"></span>.<br>
...
const PALETTE = ['#38BDF8','#34D399','#F2C572','#FC5130','#A78BFA','#22D3EE','#FB7185','#FACC15','#4ADE80','#60A5FA','#F97316'];
function animate(d, on, done){ if(REDUCE){ on(1); if(done)done(); return; } const t0=performance.now();
  (function f(now){ const t=Math.min(1,(now-t0)/d); on(easeOutCubic(t)); if(t<1) requestAnimationFrame(f); else if(done)done(); })(performance.now()); }
function countUp(id, to, fmt, dur){ const e=$(id); if(!e) return; animate(dur||1500, p=>{ e.textContent=fmt(to*p); }); }

(function(){ let h=''; for(let i=0;i<48;i++){ h+='<i style="left:'+(Math.random()*100).toFixed(2)+'%;top:'+(Math.random()*100).toFixed(2)+'%;animation-delay:'+(Math.random()*4).toFixed(2)+'s"></i>'; } $('stars').innerHTML=h; })();

setText('evento', datos.evento);
setText('subtitulo', datos.subtitulo);
$('cita').innerHTML = '«' + datos.cita + '»<b>' + datos.citaRef + '</b>';
...
function diffParts(iso){ let ms=Math.max(0,dateLocal(iso).getTime()-Date.now()); const d=Math.floor(ms/86400000); ms-=d*86400000; const h=Math.floor(ms/3600000); ms-=h*3600000; const m=Math.floor(ms/60000); ms-=m*60000; return {d,h,m,s:Math.floor(ms/1000)}; }
...
function runKpis(){ countUp('kpi-costo', datos.costoTotal, money); countUp('kpi-cuotas', datos.cuotas, money); countUp('kpi-falta', datos.brecha, money);
  animate(1500, p=>{ $('kpi-dias').textContent = nf.format(Math.round(diffParts(datos.fechaCierrePagos).d*p)); }); }

function renderPie(pieId, legId, items, fmt){
  ...
  items.forEach((it,idx)=>{ const col=PALETTE[idx%PALETTE.length]; const s=acc/total*100; acc+=it.monto; const e=acc/total*100;
    const pct=(it.monto/total*100).toFixed(1).replace('.',',');
    parts.push(col+' '+s.toFixed(2)+'% '+e.toFixed(2)+'%');
    leg+='<div class="leg-row"><span class="leg-dot" style="background:'+col+'"></span><span class="leg-l">'+it.label+'</span><span class="leg-v">'+fmt(it.monto)+' · '+pct+'%</span></div>'; });
  $(pieId).style.background='conic-gradient('+parts.join(',')+')';
  $(legId).innerHTML=leg;
}
function runEstructura(){ renderPie('pie-estructura','leg-estructura', datos.estructura, money); }
function runCosto(){ const C=datos.costoPersona;
  countUp('cp-part', C.participante.costo, money2); countUp('cp-eq', C.equipo.costo, money2);
  renderPie('pie-part','leg-part', C.participante.desglose, money2); renderPie('pie-eq','leg-eq', C.equipo.desglose, money2); }

function runCuotas(){ const P=datos.cuotaParticipante, E=datos.cuotaEquipo;
...
const RBOT=328, RTOP=78, RRANGE=RBOT-RTOP; let fuelCur=0;
function setFuel(frac){ const f=$('fuel'); f.setAttribute('y',RBOT-frac*RRANGE); f.setAttribute('height',frac*RRANGE);
  const fl=$('flame'); fl.setAttribute('transform','translate(80 330) scale('+(0.7+frac*0.6)+','+(0.4+frac*1.4)+') translate(-80 -330)'); fl.setAttribute('opacity',(0.4+frac*0.5).toFixed(2)); }
function animateFuel(t){ if(REDUCE){ setFuel(t); fuelCur=t; return; } const from=fuelCur; animate(600, p=>setFuel(from+(t-from)*p), ()=>{fuelCur=t;}); }
const simSel=new Set();
function renderSim(){ let html=''; datos.rubrosSimulador.forEach((r,i)=>{ html+='<button type="button" class="rub" data-i="'+i+'"><span class="rub-check"></span><span class="rub-n">'+r.nombre+(r.donable?' <span class="rub-tag">donable</span>':'')+'</span><span class="rub-m">'+money(r.monto)+'</span></button>'; });
  $('sim-list').innerHTML=html;
  $('sim-list').addEventListener('click', e=>{ const b=e.target.closest('.rub'); if(!b) return; const i=+b.dataset.i; if(simSel.has(i)){simSel.delete(i); b.classList.remove('on');} else {simSel.add(i); b.classList.add('on');} recomputeSim(); });
  $('sim-realista').addEventListener('click', ()=>preset(true)); $('sim-reset').addEventListener('click', ()=>preset(false)); }
function preset(don){ simSel.clear(); if(don) datos.rubrosSimulador.forEach((r,i)=>{ if(r.donable) simSel.add(i); });
  document.querySelectorAll('.rub').forEach(b=>b.classList.toggle('on', simSel.has(+b.dataset.i))); recomputeSim(); }
function recomputeSim(){ let S=0; datos.rubrosSimulador.forEach((r,i)=>{ if(simSel.has(i)) S+=r.monto; });
  const cubierto=datos.cuotas+S, falta=datos.costoTotal-cubierto; setText('sim-cubierto', money(cubierto));
  const fEl=$('sim-falta');
  ...
  setText('sim-porcabeza', money(Math.max(0,falta)/100)); animateFuel(Math.max(0,Math.min(1,cubierto/datos.costoTotal))); }

const AREA_COLOR = {'Guías':'#34D399','Música':'#F2C572','Cocina':'#38BDF8','Directores':'#FC5130'};
function renderAreas(){ const g=$('areas-grid'); datos.corresponsabilidad.forEach(a=>{ const col=AREA_COLOR[a.area]||'#34D399';
  const c=document.createElement('div'); c.className='area-card'; c.style.borderTopColor=col;
  c.innerHTML='<div class="area-h"><span class="area-name" style="color:'+col+'">'+a.area+'</span><span class="area-amt">≈ '+money(a.monto_indicativo)+'</span></div><ul class="area-items">'+a.items.map(x=>'<li>'+x+'</li>').join('')+'</ul>'; g.appendChild(c); }); }

function renderPresupuesto(){ const wrap=$('pres-list'); let html='';
  datos.presupuesto.forEach(grp=>{ html+='<details class="pres-grp"><summary><span class="pg-n">'+grp.rubro+'</span><span class="pg-c">'+grp.items.length+' ít.</span><b class="pg-s">'+money(grp.subtotal)+'</b></summary>';
    if(grp.items.length){ html+='<div class="pres-items">'; grp.items.forEach(it=>{ html+='<div class="pres-it"><span class="pi-n">'+it.item+'</span><span class="pi-d">'+it.detalle+'</span><b class="pi-m">'+money(it.monto)+'</b></div>'; }); html+='</div>'; }
    html+='</details>'; });
  wrap.innerHTML=html;
  setText('pres-sub', money(datos.subOperativo)); setText('pres-total', money(datos.costoTotal)); }
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  mkCountdown('cpago', datos.fechaCierrePagos); mkCountdown('ret', datos.fechaRetiro);
  renderAreas(); renderSim(); renderPresupuesto();
  onVisible($('kpis'), runKpis);
  onVisible($('estructura-panel'), runEstructura);
...
print("OK  tablero_campana_etc88.html  (Misión 88 · cita Sal 19,1 · estructura + presupuesto desplegable)")
print(f"    cita: «{cita['valor']}» {cita['cita']}")
print(f"    estructura: {len(datos['estructura'])} rubros (pastel) suman {sum(r['monto'] for r in sim['items']):,}")
print(f"    presupuesto desplegable: {len(presupuesto)} rubros · subtotal operativo {sub_operativo:,.0f} (Maestro)")
for g in presupuesto:
    print(f"       - {g['rubro']:42} {len(g['items']):>2} ít.  {g['subtotal']:>12,.2f}")
